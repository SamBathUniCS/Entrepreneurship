...
export default function FriendsScreen() {
  ...
  async function doSearch() {
    ...
    const r = await apiFetch(
      "GET",
      `/friends/search?q=${encodeURIComponent(searchQ)}`,
      token,
    );
    setLoading(false);
  ...
  async function declineRequest(reqId: string) {
    ...
    const r = await apiFetch(
      "POST",
      `/friends/requests/${reqId}/decline`,
      token,
    );
    setActionId(null);
    ...
    <>
      ...
      <SafeAreaView style={styles.safe}>
            ...
            >
              ...
              {t.key === "requests" && requests.length > 0 && (
                <View style={styles.badge}>
                  <Text style={styles.badgeTxt}>{requests.length}</Text>
                </View>
              )}
            </TouchableOpacity>
          ))}
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={styles.row}>
                ...
                <TouchableOpacity
                  onPress={() => unfriend(item.friendship_id, item.username)}
          ...
          <View style={{ flex: 1 }}>
            ...
            <FlatList
              ...
              ListEmptyComponent={
                searchQ.length > 1 ? (
                  <View style={styles.emptyWrap}>
                    <Text style={styles.emptyTxt}>No results.</Text>
                  </View>
                ) : (
                  <View style={styles.emptyWrap}>
                    <Text style={styles.emptyTxt}>
              ...
              renderItem={({ item }) => {
                ...
                return (
                  <View style={styles.row}>
                    ...
                    {isFriend ? (
                      <View style={styles.friendedPill}>
                        <Text style={styles.friendedTxt}>Friends ✓</Text>
                      </View>
                    ) : isPending ? (
        ...
        {tab === "leaderboard" && (
          <FlatList
            data={[...friends].sort(
              (a, b) => (b.total_uploads ?? 0) - (a.total_uploads ?? 0),
            )}
            keyExtractor={(f) => f.id}
            contentContainerStyle={styles.list}
            ListHeaderComponent={
              <Text style={styles.lbHeader}>Friends by Uploads</Text>
            }
            ListEmptyComponent={
              <View style={styles.emptyWrap}>
                <Text style={styles.emptyTxt}>
            ...
            renderItem={({ item, index }) => (
              <View style={[styles.row, index === 0 && styles.rowGold]}>
                <View
                  style={[styles.rankPill, index === 0 && styles.rankPillGold]}
                >
                  <Text
                    style={[styles.rankTxt, index === 0 && { color: "#fff" }]}
                  >
                    {index + 1}
                  </Text>
                </View>
