...
export default function CreateScreen() {
  ...
  async function poll(renderId: string) {
    ...
    const r = await apiFetch(
      "GET",
      `/events/${eventId}/renders/${renderId}`,
      token,
    );
    if (!r.ok) return;
  ...
  const isRendering =
    status === "queued" || status === "fetching" || status === "rendering";
    ...
    <>
      ...
      >
            ...
            <View style={styles.musicRow}>
              {MUSIC_OPTIONS.map((opt) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.musicChip,
                    music === opt.key && styles.musicChipActive,
                  ]}
                ...
                >
                  ...
                  <Text
                    style={[
                      styles.musicLabel,
                      music === opt.key && styles.musicLabelActive,
                    ]}
                  >
                    {opt.label}
                  </Text>
        ...
        {status !== "done" && (
          ...
          >
            ...
            <Text style={styles.generateBtnTxt}>
              {isRendering ? "Generating…" : `Generate ${renderType === "reel" ? "Reel" : "Montage"}`}
            </Text>
          </TouchableOpacity>
        )}
        ...
        <Text style={styles.footerNote}>
          Renders are powered by Shotstack and may take up to 60 seconds.
        </Text>
      </ScrollView>
    </>
  );
...
const styles = StyleSheet.create({
  ...
  typeCardSelected: { borderColor: COLORS.primary, backgroundColor: "#EDE7F6" },
  typeTitle: {
    fontSize: FONT_SIZES.body,
    fontWeight: "800",
  ...
  musicChipActive: { borderColor: COLORS.primary, backgroundColor: "#EDE7F6" },
