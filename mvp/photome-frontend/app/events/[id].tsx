...
import {
  ...
  ActivityIndicator, Modal, Alert, Platform, FlatList,
} from "react-native";
...
interface Photo {
  ...
  original_filename: string | null; faces_detected?: number;
...
interface EventDetail {
  ...
  is_member: boolean; has_access: boolean;
  upload_threshold: number; status: string; visibility: string;
}
...
export default function EventDetail() {
  ...
  async function uploadFiles(files?: File[], uris?: string[]) {
    ...
    const uploadOne = async (form: FormData) => {
      ...
      const r = await apiFetch("POST", `/events/${id}/photos/`, token, form, true);
      if (!r.ok) Alert.alert("Upload failed", r.data?.detail ?? "Unknown error");
    };
  ...
  async function pickAndUpload() {
    ...
    if (!result.canceled && result.assets.length > 0) {
      uploadFiles(undefined, result.assets.map((a) => a.uri));
    }
  }

  if (loading) {
    return <View style={styles.center}><ActivityIndicator color="#1677ff" size="large" /></View>;
  }
  if (!event) {
    return <View style={styles.center}><Text style={styles.errTxt}>Event not found.</Text></View>;
  }

  return (
    <>
      <Stack.Screen options={{ title: event.title, headerStyle: { backgroundColor: "#1677ff" }, headerTintColor: "#fff" }} />
      <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.metaRow}>
            ...
            <View style={[styles.metaPill, { backgroundColor: event.has_access ? "#F0FDF4" : "#FFF7ED" }]}>
              <Ionicons
                name={event.has_access ? "lock-open-outline" : "lock-closed-outline"}
                size={14}
                color={event.has_access ? "#16a34a" : "#ea580c"}
              />
              <Text style={[styles.metaTxt, { color: event.has_access ? "#16a34a" : "#ea580c" }]}>
                {event.has_access ? "Unlocked" : `Upload ${event.upload_threshold} to unlock`}
              </Text>
            </View>
          </View>
        </View>
        ...
        {event.is_member && event.status === "active" && (
          <TouchableOpacity
            ...
            onPress={pickAndUpload} disabled={uploading} activeOpacity={0.85}
          >
            {uploading
              ? <><ActivityIndicator size="small" color="#fff" /><Text style={styles.uploadBtnTxt}>{uploadProgress}</Text></>
              : <><Ionicons name="cloud-upload-outline" size={18} color="#fff" /><Text style={styles.uploadBtnTxt}>Upload Photos</Text></>
            }
          </TouchableOpacity>
        )}
        ...
        <View style={styles.sectionHeader}>
          ...
          <View style={styles.sectionBadge}><Text style={styles.sectionBadgeTxt}>{event.photo_count}</Text></View>
        </View>
        ...
        {photos.length === 0 ? (
          <View style={styles.emptyBox}>
            ...
            <Text style={styles.emptyTxt}>
              {event.is_member ? "No photos yet — be the first to upload!" : "Join this event to see photos."}
            </Text>
          </View>
        ) : (
          <View style={styles.grid}>
            {photos.map((p) =>
              p.locked ? (
                ...
                <TouchableOpacity key={p.id} style={styles.thumb} onPress={() => setLightbox(p)} activeOpacity={0.85}>
                  <AuthImage
                  ...
                  {(p.tags?.length ?? 0) > 0 && (
                    <View style={styles.matchBadge}>
                      <Text style={styles.matchTxt}>{p.tags!.length} match</Text>
                    </View>
                  )}
                </TouchableOpacity>
              )
      ...
      {/* Lightbox */}
      {lightbox && (
        <Modal visible animationType="fade" transparent onRequestClose={() => setLightbox(null)}>
          <View style={styles.lbBg}>
            ...
            <View style={styles.lbMeta}>
              <Text style={styles.lbFilename}>{lightbox.original_filename ?? "Photo"}</Text>
                  ...
                  {lightbox.tags!.map((t, i) => (
                    <View key={i} style={styles.lbTag}>
                      <Text style={styles.lbTagTxt}>
                        @{t.username}{t.confidence ? ` ${Math.round(t.confidence * 100)}%` : ""}
                      </Text>
                    </View>
                  ))}
                </View>
...
const styles = StyleSheet.create({
  ...
  infoCard: {
    ...
    padding: 16, gap: 10, borderWidth: 1, borderColor: "#E5E7EB",
  },
  ...
  uploadBtn: {
    ...
    backgroundColor: "#5E35B1", borderRadius: 12, paddingVertical: 13,
  },
  ...
  grid: { flexDirection: "row", flexWrap: "wrap", paddingHorizontal: 10, gap: 6 },
  thumb: {
    width: "31.5%", aspectRatio: 1, borderRadius: 10, overflow: "hidden",
    backgroundColor: "#E5E7EB",
  },
  thumbLocked: { alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: "#E5E7EB" },
  matchBadge: {
    position: "absolute", bottom: 4, right: 4,
    backgroundColor: "#5E35B1", borderRadius: 4, paddingHorizontal: 5, paddingVertical: 2,
  },
  ...
  lbBg: { flex: 1, backgroundColor: "rgba(0,0,0,.95)", alignItems: "center", justifyContent: "center" },
  lbClose: { position: "absolute", top: 52, right: 20, padding: 8, zIndex: 10 },
  lbMeta: { marginTop: 14, alignItems: "center", gap: 8, paddingHorizontal: 24 },
  lbFilename: { fontSize: 12, color: "#9ca3af" },
  lbTags: { flexDirection: "row", flexWrap: "wrap", gap: 6, justifyContent: "center" },
  lbTag: { backgroundColor: "#EDE7F6", borderRadius: 6, paddingHorizontal: 10, paddingVertical: 3 },
